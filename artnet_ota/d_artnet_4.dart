...
class ArtnetPollReplyPacket implements ArtnetPacket {
  ...
  int get universe => this.netSwitch << 8 | this.subSwitch << 4 | this.swOut[0];
  set universe(int value){
    this.netSwitch = (value >> 8) & 0x7F;
    this.subSwitch = (value >> 4) & 0x0F;
    this.swOut[0] = (value >> 0) & 0x0F;
  }
  ...
  String toString() {
    ...
    string += "Port-Address (Universe): " + this.universe.toString() + "\n";
...
class ArtnetAddressPacket implements ArtnetPacket{
  ...
  /* No Change */
  static const universeNoChange = 0x7F;
  ...
  ArtnetAddressPacket([List<int> packet]){
    ...
    //Set no change for universe programming
    this.netSwitch = universeNoChange;
    this.subSwitch = universeNoChange;
    this.swIn.forEach((val){
      val = universeNoChange;
    });
    this.swOut.forEach((val){
      val = universeNoChange;
    });
  ...
  int get universe => (this.netSwitch << 8 | this.subSwitch << 4 | this.swOut[0]);
  set universe(int value){
    this.netSwitch = value >> 8 & netSwitchMask;
    this.subSwitch = value >> 4 & subSwitchMask;
...
class ArtnetFirmwareMasterPacket implements ArtnetPacket{
  ...
  set data(List<int> value){
    for(var i = 0; i < dataSize; i++){
      ...
      this.packet.setUint16(dataIndex + i*2, value[i], Endian.little);
    }
  }
  List<int> get dataBytes => this.packet.buffer.asUint8List(dataIndex, dataSize*2);
  ...
  String toString() {
    ...
    for(var i = 0; i < dataSize*2; i++){
      ...
      String tempString = this.dataBytes[i].toRadixString(16);
      if(tempString.length < 2) tempString = "0" + tempString;
      while(tempString.length < 3){
        tempString += " ";
      }
